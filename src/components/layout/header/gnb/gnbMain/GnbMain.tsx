    ...
    */
    ...
    return (
        <Toolbar className={clsN(`${styles.tool_bar}`, toolClsN)}>
            { isInTablet ? <LeftMenuBtn icon={<MenuIcon className={clsN(`${styles.header_menu_btn_icon}`, menuIconClsN)} onClick={onClick}/>} className={clsN(`${styles.header_menu_btn}`, menuBtnClsN)} edge='start' ariaLabel='drawer Menu' onClick={onClick} /> : null}
            ...
            <Button className={clsN(styles.login, styles.loginTitle, loginClsN)} variant={variant}>로그인</Button>
        </Toolbar>
    );
};
